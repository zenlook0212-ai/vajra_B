...
def build_canon_d_hybrid_summary_prompt(user_message: str, aspects_body: str) -> str:
    ...
    return (
        ...
        "4. 不同譯本／異譯若用字不同，僅可寫「用字不同，順序相近」，"
        "禁止寫「A 即 B」「A 等同 B」等跨譯本硬對名相。\n"
        "5. 勿輸出 thinking process；第一行直接以正文開始（可省略「【綜合回答】」標題）。\n"
        "6. 勿使用 Markdown 格式（禁止 #、**、_、--- 等）；正文純繁體中文與【T…】坐標。"
    )
...
def hybrid_summary_system_message() -> str:
    return (
        ...
        + "跨譯本僅述用字差異，禁止將異譯名相硬對為同一義項（不可寫「A 即 B」）。"
    )
